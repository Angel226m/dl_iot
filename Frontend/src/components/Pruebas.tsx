...
interface Procesamiento {
  model_used: 'legacy' | 'custom'  // 🆕 Modelo usado
  architecture: string
  encoder: string
  has_cbam?:  boolean  // 🆕 Si tiene CBAM
  tta_usado:  boolean
  tta_transforms: number
  threshold: number
  target_size: number
...
interface HealthResponse {
  status: string
  models: {
    legacy: { loaded: boolean; path: string }
    custom: { loaded: boolean; path: string }
    default: string
  }
...
const Pruebas = () => {
  ...
  // 🆕 SELECCIÓN DE MODELO
  const [selectedModel, setSelectedModel] = useState<'legacy' | 'custom'>('legacy')
  ...
  const loadBackendHealth = async () => {
    try {
      ...
      if (response.ok) {
        ...
        // Auto-seleccionar modelo disponible
        if (data.models.default === 'custom' && data.models.custom.loaded) {
          setSelectedModel('custom')
        } else if (data.models.default === 'legacy' && data.models.legacy.loaded) {
          setSelectedModel('legacy')
        } else if (data.models.custom.loaded) {
          setSelectedModel('custom')
        } else if (data.models.legacy.loaded) {
          setSelectedModel('legacy')
        }
      }
    } catch (err) {
  ...
  const analyzeImage = async () => {
    ...
    if (selectedModel === 'custom' && !backendHealth.models.custom.loaded) {
      setError('Modelo Custom no disponible.Selecciona Legacy.')
      return
    }
  ...
  const getModelBadge = (modelType: 'legacy' | 'custom') => {
    if (modelType === 'legacy') {
      return {
        name: 'Legacy',
          ...
          <div className="text-center mb-16">
            <div className="inline-flex items-center gap-2 bg-cyan-500/10 border border-cyan-500/30 rounded-full px-5 py-2 mb-6">
              ...
              <span className="text-cyan-400 text-sm font-semibold tracking-wide">
                FRONTEND v6.0 + BACKEND v5.1 DUAL MODEL 🚀
              </span>
            </div>
            ...
            <div className="mt-8 flex flex-wrap justify-center gap-4">
              <div className="inline-flex items-center gap-4 bg-slate-800/50 border border-slate-700 rounded-full px-6 py-3">
                ...
                <div className="flex gap-2">
                  <button
                    onClick={() => setSelectedModel('legacy')}
                    disabled={! backendHealth?.models.legacy.loaded}
                  ...
                  >
                    {backendHealth?.models.legacy.loaded ? '✓' : '✗'} Legacy
                  </button>
                  <button
                  ...
                  >
                    {backendHealth?.models.custom.loaded ? '✓' : '✗'} Custom
                  </button>
                </div>
              </div>
              ...
              <div className="mt-6 max-w-2xl mx-auto bg-slate-800/30 border border-slate-700 rounded-xl p-4">
                <div className="flex items-center justify-center gap-6 text-sm">
                  ...
                  <div className="text-slate-300">
                    {backendHealth.connected_devices} dispositivos
                  </div>
                  <div className="text-slate-500">•</div>
                  <div className="text-slate-300">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div className="relative group">
              ...
              <div className="relative bg-gradient-to-br from-slate-800 to-slate-900 border border-slate-700 rounded-3xl p-6 md:p-8 hover:border-cyan-500/50 transition-all duration-300">
                ...
                {/* MODAL CÁMARA LOCAL - SIN CAMBIOS */}
                {isCameraOpen && (
                  <div className="fixed inset-0 z-50 bg-black/90 flex items-center justify-center p-4">
                    <div className="relative max-w-4xl w-full">
                      <button
                ...
                {! selectedImage ?  (
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex gap-3">
                      {! isProcessing && ! result && (
                        <button
                          ...
                          className="flex-1 group/btn relative overflow-hidden disabled:opacity-50 disabled:cursor-not-allowed"
                        >
                          <div className={`absolute inset-0 bg-gradient-to-r ${getModelBadge(selectedModel).color} rounded-xl blur-xl opacity-75 group-hover/btn:opacity-100 transition duration-300`}></div>
                          <div className={`relative bg-gradient-to-r ${getModelBadge(selectedModel).color} text-white py-4 px-6 rounded-xl font-semibold transition-all duration-300 flex items-center justify-center gap-2 shadow-lg hover:scale-105`}>
                            <Zap className="w-5 h-5" />
                            Analizar con {getModelBadge(selectedModel).name} {useTTA && '+ TTA'}
                          </div>
                        </button>
                      )}
                      <button
                    ...
                    {isProcessing && (
                      <div className={`bg-gradient-to-r ${getModelBadge(selectedModel).color} bg-opacity-10 border border-blue-500/30 rounded-2xl p-6`}>
                        <div className="flex items-center gap-4 mb-4">
                          <Loader className="w-10 h-10 text-blue-400 animate-spin" />
                          <div>
                            <p className="font-bold text-blue-400 text-xl">Procesando imagen...</p>
                            <p className="text-sm text-slate-400">
                              {getModelBadge(selectedModel).desc} {useTTA && '+ TTA (6x)'}
                            </p>
                            <p className="text-xs text-green-400 mt-1">🚀 Multipart (3× más rápido)</p>
                          </div>
                        </div>
                        <div className="h-2 bg-slate-700 rounded-full overflow-hidden">
                          <div className={`h-full bg-gradient-to-r ${getModelBadge(selectedModel).color} animate-pulse rounded-full w-full`}></div>
                        </div>
                      </div>
                    )}
                      ...
                      <div className="rounded-2xl overflow-hidden border border-slate-700">
                        ...
                        <div className="bg-slate-800 p-3 text-center border-t border-slate-700">
                          <p className="text-sm text-slate-400 font-medium">
                            Resultado Procesado
                            {result.procesamiento?.tta_usado && <span className="text-cyan-400"> • TTA {result.procesamiento.tta_transforms}x</span>}
                            {result.procesamiento?.model_used && (
                              <span className={`ml-2 px-2 py-1 rounded-full text-xs font-bold ${
                                result.procesamiento.model_used === 'custom' 
              ...
              <div className="relative bg-gradient-to-br from-slate-800 to-slate-900 border border-slate-700 rounded-3xl p-6 md: p-8">
                ...
                {! result ? (
                  <div className="space-y-6">
                        ...
                        <div className="bg-slate-900/50 border border-blue-500/20 rounded-xl p-4">
                          ...
                          <ul className="space-y-2 text-slate-300 text-sm">
                            <li className="flex items-start gap-2">
                              ...
                              <span>SMP UnetPlusPlus + EfficientNet-B8</span>
                            </li>
                            <li className="flex items-start gap-2">
                              <CheckCircle className="w-4 h-4 text-blue-400 flex-shrink-0 mt-0.5" />
                              <span>Modelo estable y probado</span>
                            </li>
                            <li className="flex items-start gap-2">
                              <CheckCircle className="w-4 h-4 text-blue-400 flex-shrink-0 mt-0.5" />
                              <span>Alto rendimiento en detección</span>
                            </li>
                          </ul>
                        </div>
                        ...
                        <div className="bg-slate-900/50 border border-purple-500/20 rounded-xl p-4">
                          ...
                          <ul className="space-y-2 text-slate-300 text-sm">
                            <li className="flex items-start gap-2">
                              ...
                              <span>UNet++ + CBAM Attention + ConvNeXt-Base</span>
                            </li>
                            <li className="flex items-start gap-2">
                              <CheckCircle className="w-4 h-4 text-purple-400 flex-shrink-0 mt-0.5" />
                              <span>Dual Attention (Channel + Spatial)</span>
                            </li>
                            <li className="flex items-start gap-2">
                              <CheckCircle className="w-4 h-4 text-purple-400 flex-shrink-0 mt-0.5" />
                              <span>Nested decoder para mayor precisión</span>
                            </li>
                          </ul>
                        </div>
                      </div>
                    ...
                    <div className="bg-slate-900/50 border border-slate-700 rounded-2xl p-6">
                      <h4 className="font-bold text-cyan-400 mb-4 text-lg">🧠 Características IA</h4>
                      <ul className="space-y-3 text-slate-300 text-sm">
                        <li className="flex items-start gap-2">
                          <CheckCircle className="w-5 h-5 text-cyan-400 flex-shrink-0 mt-0.5" />
                          <span><strong>Selección de modelo</strong> en tiempo real</span>
                        </li>
                        <li className="flex items-start gap-2">
                          <CheckCircle className="w-5 h-5 text-cyan-400 flex-shrink-0 mt-0.5" />
                          <span><strong>TTA opcional</strong> con 6 transformaciones</span>
                    ...
                    <div className="bg-green-500/10 border border-green-500/30 rounded-2xl p-6">
                      <h4 className="font-bold text-green-400 mb-3 text-lg">🚀 Backend v5.1 Features</h4>
                      <ul className="space-y-2 text-slate-300 text-sm">
                        <li>• ✅ <strong>Dual Model Support</strong> (Legacy + Custom)</li>
                        <li>• ✅ Subida con <code className="bg-slate-800 px-2 py-1 rounded text-cyan-400">multipart/form-data</code></li>
                        <li>• ✅ 3× más rápido que base64 (0% overhead)</li>
                        <li>• ✅ Parámetro <code className="bg-slate-800 px-2 py-1 rounded text-purple-400">model=legacy|custom</code></li>
                    ...
                    <div className="bg-gradient-to-br from-blue-500/10 to-purple-500/10 border border-blue-500/30 rounded-2xl p-6">
                      ...
                      <ul className="space-y-2 text-slate-300 text-sm">
                        ...
                        <li>• Mantenimiento preventivo de infraestructura</li>
                        <li>• Inspección remota con Raspberry Pi</li>
                        <li>• Comparación de modelos IA (Legacy vs Custom)</li>
                      </ul>
                    </div>
                  </div>
                ) : (
                  <div className="space-y-6">
                    ...
                    {result.procesamiento?.model_used && (
                      <div className={`bg-gradient-to-r ${
                        ...
                        <p className="text-sm text-slate-300">
                          {result.procesamiento.model_used === 'custom' 
                            ? 'UNet++ con Dual Attention (CBAM) y ConvNeXt-Base encoder'
                            : 'SMP UnetPlusPlus con EfficientNet-B8 encoder'}
                        </p>
                      </div>
                    )}
                    ...
                    <div className="bg-gradient-to-br from-green-500/10 to-emerald-500/10 border border-green-500/30 rounded-2xl p-6">
                      ...
                      <p className="text-xs text-green-400 mt-2 flex items-center gap-1">
                        <ZapIcon className="w-3 h-3" />
                        Procesado con Backend v5.1 Dual Model + Multipart (3× más rápido)
                      </p>
                    </div>
                  </div>
                )}
